...
describe('PlayerFactory', function() {
	...
	describe('.getPlayers', function() {
		...
		it('should translate player names into account IDs when necessary', async function() {
			srv.get(/account\/info/).times(4).reply(200, {});
			...
			await playerFactory.getPlayers([':Bot1:',':Bot2:']);
			expect(playerFactory.getAccounts, 'bots').to.not.have.been.called;

			await playerFactory.getPlayers([1, ':Bot:', 'player2']);
			expect(playerFactory.getAccounts, 'mixed names, bots and account IDs').to.have.been.calledWith(['player2']);
		});
		...
		it('should return instances of Player over the correct data', async function() {
			...
			let result = await playerFactory.getPlayers([ 1, 2, ':Bot:' ]);
